...
* `Sabertooth 2x60 <https://www.dimensionengineering.com/products/sabertooth2x60>`_
* `Dual LS7366R Quadrature Encoder Buffer <https://www.superdroidrobots.com/shop/item.aspx/dual-ls7366r-quadrature-encoder-buffer/1523/>`_ (with `1024 P/R encoders <https://www.sparkfun.com/products/11102>`_)

Required Libraries
------------------
Copies of the required libraries are included in this repository in `/library/*`

* Note that the libraries are originally from: `Sabertooth Library <https://www.dimensionengineering.com/software/SabertoothArduinoLibraries.zip>`_ and `Encoder Buffer Library <https://github.com/SuperDroidRobots/Encoder-Buffer-Library.git>`_
...
Write a 'm' (move) and two 32 bit float speeds (one for each motor) to the Arduino
...
For more information, consult the `Sabertooth 2x60 User Guide <https://www.dimensionengineering.com/datasheets/Sabertooth2x60.pdf>`_

Encoder Interface (SPI)
-----------------------

The Arduino communicates with the Quadrature Encoder Buffer over SPI.  The buffer chip is used to count pulses for each encoder because the `1024 P/R encoders <https://www.sparkfun.com/products/11102>`_ send out pulses faster than the Arduino can count.
