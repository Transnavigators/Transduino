...
def generateIndex(inDir, outDir):
    ...
    # generate index.rst
    inFile = open(os.path.abspath(INDEX_TEMP), "r+")
    outFile = open(os.path.abspath(outDir + "/index.rst"), "w+")
    for line in inFile :
        outFile.write(line)
    inFile.close()
...
def generateRST(outDir, moduleName, listModules, listFiles) :
    ...
    for fileName in listFiles :
        ...
        outFile.write("   :project: transduino\n\n")
...
def generateRSTs(inDir, outDir, isRoot=False):
    ...
    listFiles = []
    for fileName in os.listdir(inDir) :
        if os.path.isdir(inDir + "/" + fileName) == True and fileName != ".git":
            listModules.append(fileName)
